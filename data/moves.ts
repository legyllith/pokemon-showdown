...
export const Moves: {[moveid: string]: MoveData} = {
	...
	courtchange: {
		...
		onHitField(target, source) {
			const sideConditions = [
				'mist', 'lightscreen', 'reflect', 'spikes', 'safeguard', 'tailwind', 'toxicspikes', 'stealthrock', 'waterpledge', 'firepledge', 'grasspledge', 'stickyweb', 'auroraveil', 'gmaxsteelsurge', 'stealthice', 'gmaxcannonade', 'gmaxvinelash', 'gmaxwildfire',
			];
			let success = false;
	...
	defog: {
		...
		onHit(target, source, move) {
			...
			const removeTarget = [
				'reflect', 'lightscreen', 'auroraveil', 'safeguard', 'mist', 'spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'stealthice',
			];
			const removeAll = [
				'spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'stealthice',
			];
	...
	gmaxwindrage: {
		...
		self: {
			onHit(source) {
				...
				const removeAll = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'stealthice'];
				for (const targetCondition of removeTarget) {
	...
	rapidspin: {
		...
		onAfterHit(target, pokemon) {
			...
			const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'stealthice'];
	...
	beachterrain: {
		...
		condition: {
			...
			onFieldStart(field, source, effect) {
				if (effect?.effectType === 'Ability') {
					this.add('-fieldstart', 'move: Beach Terrain', '[from] ability: ' + effect, '[of] ' + source);
				} else {
			...
			onFieldResidualSubOrder: 7,
			onFieldEnd() {
				this.add('-fieldend', 'move: Beach Terrain');
			},
		},
		secondary: null,
		target: "all",
		...
		condition: {
			...
			onModifyMove(move, attacker, defender) {
				if (['powergem', 'diamondstorm'].includes(move.id)) {
					...
					this.field.setTerrain('caveterrain') //A changer quand le terrain crystal cavern sera implémenté
				}
	...
	rainybloom: {
		...
		onBasePower(basePower, pokemon, target) {
			if (['sunnyday', 'desolateland', 'sandstorm', 'hail'].includes(pokemon.effectiveWeather())) {
				this.debug('weakened by weather');
				return this.chainModify(0.5);
			}
		},
	...
	tropicalhurrican: {
		...
		priority: 0,
			self: {
				onHit(source) {
					if (source.hasType('Grass') && !source.hasType('Fire')){source.boost({spa: 1}); source.setType(source.getTypes(true).map(type => type === "Grass" ? "Fire" : type));}
					if (source.hasType('Fire') && !source.hasType('Grass')){source.boost({atk: 1}); source.setType(source.getTypes(true).map(type => type === "fire" ? "Grass" : type));}
