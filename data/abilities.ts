...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	sapofimmortality: {
		...
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Poison' && move.category !== 'Status') {
	...
	walkman: {
		...
		onSourceTryPrimaryHit(target, source, effect) {
			...
			if (
				...
				if (source.species.id !== 'paritonecombat') source.formeChange('Paritone-Combat', effect);
			}
			else if (
	...
	drakonize: {
		...
		onModifyType(move, pokemon) {
			...
			if (move.type === 'Normal' && !noModifyType.includes(move.id) && !(move.isZ && move.category !== 'Status')) {
				move.type = 'Dragon';
				move.drakonizeBoosted = true;
			}
		},
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			if (move.drakonizeBoosted) {
				return this.chainModify([4915, 4096]);
			}
		},
		name: "Drakonize",
	...
	cudchew: {
		...
		onResidual(pokemon) {
			if (pokemon.hp && !pokemon.item && this.dex.items.get(pokemon.lastItem).isBerry) {
					...
					if (pokemon.lastItem.onEat) source.ateBerry = true;
					pokemon.lastItem = '';
					//this.add('-item', pokemon, pokemon.getItem(), '[from] ability: Harvest');
